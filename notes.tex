...
\begin{document}
	...
	\section{Estruturas de Dados}
		\subsection{Union-Find}
			...
			\lstinputlisting{algoritmos/structs/union-find.cpp}
		\subsection{Fenwick Tree}
			Tempo: \( O(\log n) \) em todas as operações
			\lstinputlisting{algoritmos/structs/fenwick.cpp}
		\subsection{Suffix Array}
			Tempo: \( O(n \log n) \)
			\lstinputlisting{algoritmos/structs/suffixarray.cpp}

	\section{Matemática}
		\subsection{Useful Primes}
			\lstinputlisting{algoritmos/math/useful.cpp}
		\subsection{Bigmod}
			\[ r = b^p \bmod m \]
			\lstinputlisting{algoritmos/math/bigmod.cpp}
		\subsection{Long Integer}
			\lstinputlisting{algoritmos/math/bigint.cpp}
		\subsection{Decomposição em Fatores Primos}
			\lstinputlisting{algoritmos/math/primefact.cpp}
		\subsection{GCD e LCM}
			\lstinputlisting{algoritmos/math/gcdlcm.cpp}
		\subsection{Algoritmo de Euclides}
			\lstinputlisting{algoritmos/math/euclides.cpp}
		\subsection{ax == b (mod m)}
			\lstinputlisting{algoritmos/math/axbmodm.cpp}
		\subsection{Combinação}
			\[ C_n^k = {n! \over {k! \cdot (n - k)!}} \]
			\lstinputlisting{algoritmos/math/comb.cpp}
		\subsection{Floyd's Cycle-Finding}
			\lstinputlisting{algoritmos/math/floydcycle.cpp}
		\subsection{Matrizes}
			\subsubsection{Decomposição LUP}
				Determina uma decomposição LUP para a matriz \(A\): \( PA = LU \)
				Tempo: \( \Theta(n^3) \)
				\lstinputlisting{algoritmos/math/lup-decomposition.cpp}
			\subsubsection{Determinante}
				\lstinputlisting{algoritmos/math/determinante.cpp}
			\subsubsection{Sistemas Lineares}
				Resolve um sistema de equações lineares da forma \(Ax = B\)
				\lstinputlisting{algoritmos/math/sistlinear.cpp}
			\subsubsection{Eliminação Gaussiana}
				\lstinputlisting{algoritmos/math/gausselim.cpp}
		\subsection{Números Complexos}
			\lstinputlisting{algoritmos/math/complex.cpp}
		\subsection{Durand-Kerner}
			Encontra as raízes de um polinômio:
			\[ a_0 x^n + a_1 x^{n-1} + \cdot + a_{n-1} x + a_n = 0 \]
			\lstinputlisting{algoritmos/math/durand-kerner.cpp}
		\subsection{Polinômios}
			\lstinputlisting{algoritmos/math/polynomial.cpp}
		\subsection{Gerador de Triplas Pitagóricas}
			\lstinputlisting{algoritmos/math/pythgen.cpp}

	\section{Grafos}
		\subsection{Strongly Connected Components}
			\subsubsection{Kosaraju}
				Tempo: \( O(V + E) \)
				\lstinputlisting{algoritmos/graph/kosaraju.cpp}
			\subsubsection{Tarjan}
				\lstinputlisting{algoritmos/graph/tarjan.cpp}
		\subsection{Caminho mínimo}
			\subsubsection{Dijkstra}
				Tempo: \( O((|E| + |V|) \log |V|) \)
				\lstinputlisting{algoritmos/graph/dijkstra.cpp}
			\subsubsection{Bellman-Ford}
				Tempo: \( O(|V| |E|) \)
				\lstinputlisting{algoritmos/graph/bellman-ford.cpp}
		\subsection{Kruskal}
			Tempo: \( O(|E| \log |V|) \) ou \( O(|E| \ldots \alpha(|V|)) \) caso as arestas estejam pré-ordenadas.
			\lstinputlisting{algoritmos/graph/kruskal.cpp}
		\subsection{Fluxo Máximo e Corte Mínimo}
			\subsubsection{Edmonds-Karp}
				Tempo: \( O(|V| |E|^2) \)
				\lstinputlisting{algoritmos/graph/ford-fulkerson.cpp}
			\subsubsection{Stoer-Wagner}
				Algoritmo para encontrar o corte mínimo (não s-t) em um grafo. A implementação abaixo é \(O(n^3)\).
				\lstinputlisting{algoritmos/graph/stoer-wagner.cpp}
			\subsubsection{Min-Cost Max-Flow}
				Tempo: \( O(min(n^2 m \cdot fcost, ~ n m \cdot flow) ) \)
				\lstinputlisting{algoritmos/graph/mincostmaxflow.cpp}
			\subsubsection{Kuhn-Munkres}
				...
				Tempo: \( O(n^3) \)
				\lstinputlisting{algoritmos/graph/kuhn-munkres.cpp}
		\subsection{Bipartite Matching}
			\subsection{Hopcroft-Karp}
				Tempo: \( O(|E| \sqrt{|V|})\)
				\lstinputlisting{algoritmos/graph/hopcroft-karp.cpp}
			\subsection{Kuhn}
				Tempo: \( O(|V| |E|) \)
				\lstinputlisting{algoritmos/graph/kuhn.cpp}
		\subsection{Aresta de Corte, etc}
			\lstinputlisting{algoritmos/graph/bridge.cpp}
		\subsection{Lowest Common Ancestor}
			% TODO: Marin, coloque o teu LCA do TopCoder aqui
			Tempo: \( O(V + Q + \alpha(V + Q)) \)
			\lstinputlisting{algoritmos/graph/lca.cpp}
		\subsection{Ciclo Euleriano}
			\lstinputlisting{algoritmos/graph/ciclo-euleriano.cpp}
	...
	\section{Programação Dinâmica}
		\subsection{Longest Common Subsequence}
			Tempo: \( O(nm) \)
			\lstinputlisting{algoritmos/dp/lcs.cpp}
		\subsection{Longest Increasing Subsequence}
			Tempo: \( O(n \log n) \)
			\lstinputlisting{algoritmos/dp/lis.cpp}
		%\subsection{Smallest Common Supersequence}
		%	% TO DO
		\subsection{Matrix-chain Multiplication}
			Tempo: \( O(n^3) \)
			\lstinputlisting{algoritmos/dp/arraymultiplication.cpp}
		\subsection{Damerau-Levenshtein}
			\lstinputlisting{algoritmos/dp/damlev.cpp}
		\subsection{Range Minimum Query}
			\lstinputlisting{algoritmos/dp/rmq.cpp}
		%\subsection{CYK}
		%	Tempo: \( O(n^3) \)
		%	% TO DO	
	\section{Geometria Computacional}
		\lstinputlisting{algoritmos/geometry/geometry-defs.cpp}
		\subsection{Geometria Básica 2D}
			\lstinputlisting{algoritmos/geometry/geo2d.cpp}
		\subsection{Intersecção de Segmentos}
			\lstinputlisting{algoritmos/geometry/segments-intersect.cpp}
		\subsection{Ponto dentro de Polígono}
			...
			\lstinputlisting{algoritmos/geometry/inpoly.cpp}
		...
		\subsection{Área de um Polígono}
			...
			\subsubsection{Convexo}
				...
				\lstinputlisting{algoritmos/geometry/convex-area.cpp}
			\subsubsection{Qualquer}
				\lstinputlisting{algoritmos/geometry/poly-area.cpp}
		\subsection{Convex Hull}
			\subsubsection{Graham Scan}
				Tempo: \(O(n \log n)\)
				\lstinputlisting{algoritmos/geometry/graham-scan.cpp}
			\subsubsection{Jarvis March}
				Tempo: \(O(n h)\)
				\lstinputlisting{algoritmos/geometry/jarvis-march.cpp}
		\subsection{Closest Pair of Points}
			\lstinputlisting{algoritmos/geometry/closest-pair.cpp}
	...
	\section{Outros}
		...
		\subsection{Kadane 1D}
			Tempo: \(\Theta(n)\)
			\lstinputlisting{algoritmos/others/kadane1d.cpp}
		\subsection{Kadane 2D}
			Tempo: \(O(n^3)\)
			\lstinputlisting{algoritmos/others/kadane2d.cpp}
		\subsection{Majority Problem}
			Encontra o elemento que aparece em mais da metade de um array em \(\Theta(n)\)
			\lstinputlisting{algoritmos/others/majority.cpp}
		\subsection{Stable Marriage}
			...
			\subsubsection{Gale-Shapley}
				Tempo: \(O(n^2)\)
				\lstinputlisting{algoritmos/others/stablemarriage.cpp}
		\subsection{Números Romanos}
			\lstinputlisting{algoritmos/others/romanos.cpp}
		\subsection{Calendário}
			\lstinputlisting{algoritmos/others/calendario.cpp}
		\subsection{Josephus Problem}
			\lstinputlisting{algoritmos/others/josephus.cpp}
		\subsection{KMP}
			\lstinputlisting{algoritmos/others/kmp.cpp}
		\subsection{KMP 2D}
			\lstinputlisting{algoritmos/others/kmp2d.cpp}
		\subsection{Aho-Corasick}
			\lstinputlisting{algoritmos/others/ahocorasick.cpp}
		\subsection{Alpha-Beta Pruning}
			\lstinputlisting{algoritmos/others/alphabeta.cpp}
		\subsection{Grids}
			\lstinputlisting{algoritmos/others/grids.cpp}
